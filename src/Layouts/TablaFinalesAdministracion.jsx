...
const TablaFinalesAdministracion = ({ color, busqueda, estadoFiltro, buscador }) => {
...
{/* TODO */}
  ...
  const headers = ["Materia", "Titular", "Fecha", "Horario", "Capacidad", "Cerrar mesa", "Modificar"];
    ...
    <div className={`gap-8 flex flex-col rounded-xl bg-white p-8`}>
      ...
      <div className="flex flex-row justify-between items-center">
        ...
        <div className="flex flex-col justify-between items-end w-[48%] gap-8">
          <AddFinalModal fetchAlumnos={fetchAlumnos} /> {/* TODO */}
          {buscador}
        </div>
      </div>
      ...
      <Tabla headers={headers} color={color}>
        {alumnosFiltrados.map((alumno, index) => (
          <tr key={index}>
            <td className="py-3 px-5 text-center">Algebra</td>
            <td className="py-3 px-5 text-center">Campos Jose</td>
            <td className="py-3 px-5 text-center">20/12/2024</td>
            <td className="py-3 px-5 text-center">8:00</td>
            <td className="py-3 px-5 text-center">0/20</td>
            <td className="py-3 px-5 text-center">
              <button 
                  onClick={() => toggleRegularStatus(alumno.id)}
                  className={`px-4 py-2 rounded-full ${alumno.regular ? "aceptar" : "otro-button"}`}
              >
                  {alumno.regular ? "Abierta" : "Cerrada"}
              </button>
          </td>
...
export default TablaFinalesAdministracion;
