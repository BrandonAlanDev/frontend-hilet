...
export async function UpdateAlumno(alumnoData) {
    // Validar que todos los datos estén presentes antes de enviar la solicitud
    ...
    for (const field of requiredFields) {
        if (!alumnoData.hasOwnProperty(field)) {
            throw new Error(`Falta el campo requerido: ${field}`);
        }
    }

    const token = sessionStorage.getItem('jwtToken');
    const fullUrl = `${backendurl}UsuarioController/Update`;
